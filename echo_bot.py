...
class RequestHandler(BaseHTTPRequestHandler):
    def do_POST(self):
        ...
        # 校? verification token 是否匹配，token 不匹配?明?回?并非?自??平台
        token = obj.get("token", "")
        ...
        # 根据 type ?理不同?型事件
        type = obj.get("type", "")
        if "url_verification" == type:  # ???求 URL 是否有效
            self.handle_request_url_verify(obj)
        elif "event_callback" == type:  # 事件回?
            # ?取事件?容和?型，并?行相??理，此?只?注?机器人推送的消息事件
            event = obj.get("event")
    ...
    def handle_message(self, event):
        ...
        # ?用?消息 API 之前，先要?取 API ?用??：tenant_access_token
